...
import { customTerrainBindings } from '../../src/custom/content/bindings.js';
import {
  customMaterialBindings,
  MATERIAL_COLUMNS,
  MATERIAL_STRIDE,
  MATERIAL_TILES,
  materialTileCoords,
  TRANSITION_CORNERS,
} from '../../src/custom/content/material-layout.js';
import { customTerrainMaterials } from '../../src/custom/content/materials.js';
...
describe('own meadow material pack', () => {
  it('keeps mountain ridges continuous across triangle, row and source-page boundaries', () => {
    ...
    const ground = customMaterialBindings(rows, [], customTerrainMaterials).ground;
  ...
  it('joins selected pages and exact names without claiming other entries on a shared page', () => {
    const result = customMaterialBindings(
      [
  ...
  it('keeps six directional pairs and both authored variants distinct, with missing overlays visible', () => {
    ...
    const textures = customTerrainBindings(new Map(), result.ground, result.transitions);
