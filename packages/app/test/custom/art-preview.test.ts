...
import { artPreviewPlugin } from '../../vite/custom/art-preview.js';
...
async function fixture() {
  const root = await realpath(await mkdtemp(join(tmpdir(), 'custom-art-preview-')));
  roots.push(root);
  const app = join(root, 'packages/app');
  const own = join(app, 'src/assets/custom');
  const preview = join(root, '.art-build/terrain/sample/preview/custom');
  ...
  await put(
    ...
    `
import manifest from './assets/custom/props/old/runtime.json';
const images = import.meta.glob('./assets/custom/props/*/*.png', { eager: true, query: '?url', import: 'default' });
const url = new URL('./assets/custom/props/old/image.png', import.meta.url).href;
const ui = new URL('./assets/ui/foundation/icons.png', import.meta.url).href;
globalThis.result = { manifest, images, url, ui };`,
  );
...
describe('candidate asset preview', () => {
  it('resolves static JSON, newly added glob entries and new URL from the overlay', async () => {
    ...
    try {
      ...
      expect(result?.code).toContain(`new URL("${previewUrl}/ui/foundation/icons.png"`);
  ...
  it('loads a candidate-only terrain manifest and image through the actual material loader', async () => {
    const { root, app, preview } = await fixture();
    const folder = join(app, 'src/custom/content');
    await mkdir(folder, { recursive: true });
    await writeFile(
      join(folder, 'materials.ts'),
      await readFile(new URL('../../src/custom/content/materials.ts', import.meta.url)),
    );
    const server = await createServer({
      configFile: false,
      root: app,
      ...
      resolve: {
        alias: {
          '@open-northland/art-contracts/custom': fileURLToPath(
            import.meta.resolve('@open-northland/art-contracts/custom'),
          ),
        },
      },
      logLevel: 'silent',
    ...
    try {
      const loaded = await server.ssrLoadModule('/src/custom/content/materials.ts');
      expect(loaded.customTerrainMaterials).toEqual([
        {
          id: 'mud',
          image: 'new-mud.png',
          tint: [1, 1, 1],
      ...
      expect(loaded.customMaterialUrls.get('new-mud.png')).toContain('/terrain/new-mud.png');
    } finally {
      await server.close();
    }
  });
