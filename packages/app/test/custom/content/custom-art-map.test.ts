...
describe.runIf(hasRealIr() && existsSync(path))('custom art on the local tutorial meadow', () => {
  it('covers every used pattern and retains source terrain lanes', () => {
    ...
    const patterns = customGrassBindings(ir.gfxPatterns, 512, 512);
    const patch = customMapPatch(map, { x: 130, y: 70, width: 18, height: 20 }, patterns);
