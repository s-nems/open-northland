...
import { customGrassBindings, customMapPatch } from '../../src/custom/art-review/map-patch.js';
...
const bindings = customGrassBindings([row], 512, 512);
...
describe('custom art map patch', () => {
  ...
  it('rejects unsupported patterns and active overlays instead of hiding them', () => {
    expect(() => customMapPatch(map, area, new Map())).toThrow('Unsupported ground patterns');
    const transitions = map.transitions;
    if (!transitions) throw new Error('Missing fixture transitions');
    const a1 = [...transitions.a1];
    a1[13] = 0;
    expect(() => customMapPatch({ ...map, transitions: { ...transitions, a1 } }, area, bindings)).toThrow(
      'transition overlays',
    );
  });
  it('rejects invalid bounds and odd starting rows', () => {
    for (const changed of [{ y: 3 }, { x: 5 }, { width: 0 }, { x: 1.5 }, { y: -2 }]) {
      expect(() => customMapPatch(map, { ...area, ...changed }, bindings)).toThrow('Invalid patch bounds');
    }
  });
});
