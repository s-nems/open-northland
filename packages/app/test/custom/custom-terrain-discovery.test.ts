...
import { customMaterialUrls, customTerrainMaterials } from '../../src/custom/content/materials.js';
  ...
  it('retains every delivered material and its calibration, excluding legacy bindings', () => {
    const expected = ['materials.json', 'rock-materials.json', 'sand-materials.json'].flatMap((name) => {
      const raw: unknown = JSON.parse(
        readFileSync(new URL(`../../src/assets/custom/terrain/${name}`, import.meta.url), 'utf8'),
      );
      return collectTerrainMaterials([raw]);
    });
    expect(customTerrainMaterials).toEqual(expect.arrayContaining(expected));
    for (const entry of expected) expect(customMaterialUrls.has(entry.image)).toBe(true);
  });
