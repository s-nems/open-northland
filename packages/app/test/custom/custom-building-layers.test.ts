...
import { loadCustomBuildingLayers } from '../../src/custom/content/building-layers.js';
import { customBuildingManifest } from '../../src/custom/content/building-manifest.js';
...
const manifest = customBuildingManifest.parse({
  tribeId: 1,
  typeId: 1,
  layer: 'fixture',
  sprite: 'body.png',
...
describe('own construction layer loading', () => {
  it('aligns a larger shadow canvas without changing body bounds or construction layers', async () => {
    ...
    const layers = await loadCustomBuildingLayers({ ...manifest, shadow }, (name) => name);
  ...
  it('rejects missing or mismatched shadow before returning a partial building', async () => {
    ...
    await expect(
      loadCustomBuildingLayers(withGround, (name) => (name === 'shadow.png' ? undefined : name)),
    ).rejects.toThrow('Missing sprite shadow.png');
    await expect(loadCustomBuildingLayers(withGround, (name) => name)).rejects.toThrow('dimensions');
  });
  it('keeps the final body untimed and the stage aligned with its mask', async () => {
    const layers = await loadCustomBuildingLayers(manifest, (name) => `/assets/${name}`);
  ...
  it('rejects missing or mismatched masks instead of binding an unmasked finished body as a stage', async () => {
    await expect(
      loadCustomBuildingLayers(manifest, (name) => (name === 'time.png' ? undefined : name)),
    ).rejects.toThrow('mask');
    mocks.pixels.mockResolvedValue({ width: 1, height: 1, data: new Uint8ClampedArray(4) });
    await expect(loadCustomBuildingLayers(manifest, (name) => name)).rejects.toThrow('mask');
  });
  it('rejects artwork with a different canvas before publishing layers', async () => {
    ...
    await expect(loadCustomBuildingLayers(manifest, (name) => name)).rejects.toThrow('dimensions');
  });
});
...
describe('own overlay sheet loading', () => {
  ...
  it('loads the sheet as a separate frame-indexed layer beside the body', async () => {
    ...
    const layers = await loadCustomBuildingLayers({ ...manifest, overlay }, (name) => name);
  ...
  it('rejects a sheet whose image does not hold every frame', async () => {
    ...
    await expect(loadCustomBuildingLayers({ ...manifest, overlay }, (name) => name)).rejects.toThrow(
      'dimensions',
    );
  });
});
