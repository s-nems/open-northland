...
import { galleryMapDestination } from '../src/entries/art-gallery/locations.js';

describe('gallery map destinations', () => {
  it('opens the documented real farm placement using own assets at world review scale', () => {
  ...
  it('uses the existing appearance override only when a character is selected', () => {
    ...
    expect(new URLSearchParams(character.href).get('ownHead')).toBe('man-silver');
    const normal = galleryMapDestination({ id: 'characters/man-silver', kind: 'character' });
    expect(new URLSearchParams(normal.href).has('ownHead')).toBe(false);
