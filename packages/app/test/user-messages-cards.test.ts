...
describe('notice cards', () => {
  ...
  it('draws a live settler subject and swaps in the swords for an attacked one', () => {
    ...
    expect(noticeThumb(USER_MESSAGE_TYPE.hungry, settler)).toEqual({ kind: 'settler', entity: 7 });
    expect(noticeThumb(USER_MESSAGE_TYPE.humanAttacked, settler)).toEqual({
      kind: 'glyph',
      glyph: 'swords',
      dim: false,
    });
  ...
  it('shows a glyph for a building or a subjectless row, dim for a death', () => {
    const building = { kind: 'building', entity: 3 } as const;
    expect(noticeThumb(USER_MESSAGE_TYPE.houseFinished, building)).toEqual({
      kind: 'glyph',
      glyph: 'house',
      dim: false,
    });
    expect(noticeThumb(USER_MESSAGE_TYPE.houseAttacked, building)).toEqual({
      kind: 'glyph',
      glyph: 'swords',
      dim: false,
    });
    expect(noticeThumb(USER_MESSAGE_TYPE.humanDied, null)).toEqual({
      kind: 'glyph',
      glyph: 'skull',
      dim: true,
    });
    expect(noticeThumb(USER_MESSAGE_TYPE.playerSighted, null)).toEqual({
      kind: 'glyph',
      glyph: 'banner',
      dim: false,
    });
    expect(noticeThumb(USER_MESSAGE_TYPE.specialItemFound, null)).toEqual({
      kind: 'glyph',
      glyph: 'chest',
      dim: false,
    });
    expect(noticeThumb(USER_MESSAGE_TYPE.experienceUnlocks, null)).toEqual({
      kind: 'glyph',
      glyph: 'scroll',
      dim: false,
    });
