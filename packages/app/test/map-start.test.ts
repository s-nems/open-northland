...
function entity(
  ...
  return { id, components } as SnapshotEntity;
}

function snapshot(entities: SnapshotEntity[]): WorldSnapshot {
...
describe('mapStartFocus', () => {
  ...
  it('falls back to ANY placed entity on a foreign-owned-only map', () => {
    const focus = mapStartFocus(
      snapshot([entity(1, 'settler', 40, 40, ENEMY), entity(2, 'building', 60, 40, ENEMY)]),
      256,
      256,
    );
  ...
  it('falls back to the map centre when no settler/building is placed (resources never pull focus)', () => {
    ...
    expect(mapStartFocus(snapshot([entity(1, 'resource', 10, 10)]), 100, 80)).toEqual({ x: 50, y: 40 });
    expect(mapStartFocus(snapshot([]), 100, 80)).toEqual({ x: 50, y: 40 });
  });
});
