...
import { TERRAIN_BLOCKED, TERRAIN_MARGIN, TERRAIN_OPEN, TERRAIN_WATER } from '../src/game/sandbox/index.js';

/**
 * The decoded-map → sim collision join (content/collision.ts): synthetic fixtures shaped like the
 * real lanes (ground triangle patterns by name → logicType; object placements by name → the
 * `[GfxLandscape]` walk/build areas), asserting each source lands in its semantic terrain class.
 */

/** A 6×4 map: meadow ground except one water and one mountain cell, plus one placed tree. */
function fixtureMap() {
  const W = 6;
  const H = 4;
  const meadow = 0;
  ...
  return parseTerrainMap({
    ...
    ground: { patterns: ['meadow 01', 'water 01', 'mountain 01'], a, b },
    objects: {
  ...
  landscapeGfx: [
    {
      ...
      walkBlockAreas: [
        [3, 0, 0, 1],
        [1, 0, 0, 1], // a lower state's row — must be ignored (full-state collapse)
      ],
      buildBlockAreas: [
        [3, -1, -1, 3],
        [3, -1, 0, 3],
...
describe('buildCollisionTerrain', () => {
  ...
  it('blocks a cell when EITHER ground triangle is water/mountain', () => {
    expect(at(1, 1)).toBe(TERRAIN_WATER); // water on triangle A only
    expect(at(4, 2)).toBe(TERRAIN_WATER); // mountain on triangle B only
  });
