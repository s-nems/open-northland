...
function nextIndex(dir: string, slug: string): number {
  const prefix = `${slug}-`;
  let highest = 0;
...
export function storeReport(dir: string, report: BenchReport): string {
  ...
  const index = `${nextIndex(dir, slug)}`.padStart(INDEX_DIGITS, '0');
  const rev = report.environment.rev ?? 'nogit';
  const dirty = report.environment.dirty === true ? '-dirty' : '';
  return writeReport(join(dir, `${slug}-${index}-${rev}${dirty}.json`), report);
}
