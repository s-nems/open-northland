...
import { fetchImageData } from '../../content/net.js';
import {
  type CustomBuildingManifest,
  customBuildingAtlas,
  customConstructionLayer,
  customOverlayAtlas,
  customOverlayLayer,
  customOverlaySheetSize,
} from './building-manifest.js';

export async function loadCustomBuildingLayers(
  ...
  async function load(
    sprite: string,
    timeMask?: string,
    geometry: CustomBuildingManifest = manifest,
  ): Promise<SpriteLayer> {
    const { source } = await texture(sprite, geometry);
    const layer = { source, atlas: customBuildingAtlas(geometry) };
  ...
  const stateOverlay =
    overlay === undefined
      ? {}
      : {
          [customOverlayLayer(manifest)]: {
            source: (await texture(overlay.sprite, customOverlaySheetSize(overlay))).source,
            atlas: customOverlayAtlas(manifest, overlay),
          },
        };
  const stages = await Promise.all(
    (manifest.construction ?? []).map(
      async (stage, i) =>
        [customConstructionLayer(manifest, i), await load(stage.sprite, stage.timeMask)] as const,
    ),
  );
