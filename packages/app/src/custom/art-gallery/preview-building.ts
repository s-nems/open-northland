...
import { loadCustomBuildingLayers } from '../content/building-layers.js';
import {
  customBuildingBindings,
  customLayerScale,
  customOverlayLayer,
} from '../content/building-manifest.js';
...
/** One custom layer's frame 0 as a sprite at its manifest offset, scaled to world px; textureless without
...
export async function buildingPreview(
  ...
  const layers = await loadCustomBuildingLayers(manifest, (filename) => files.get(filename));
  const person = reference === undefined ? undefined : await characterPreview(reference);
  const binding = customBuildingBindings(0, [manifest]);
  ...
  for (const [key, layer] of Object.entries(layers)) {
    for (const frame of [layer.atlas.frames.get(0), layer.shadow?.atlas.frames.get(0)]) {
      if (frame !== undefined) box = unionBox(box, frameBox(frame, customLayerScale(manifest, key)));
    }
  }
  ...
  for (const [key, layer] of Object.entries(layers)) {
    const sprite = placedOwn(layer, customLayerScale(manifest, key));
    if (sprite === undefined) continue;
    sprites[key] = sprite;
    anchor.addChild(sprite);
  }
  const spinLayer = manifest.overlay === undefined ? undefined : layers[customOverlayLayer(manifest)];
  const spinSprite = manifest.overlay === undefined ? undefined : sprites[customOverlayLayer(manifest)];
  const spin =
