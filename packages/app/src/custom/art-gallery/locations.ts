...
const reviewLocations: Readonly<Record<string, ReviewLocation | SceneLocation>> = {
  ...
  'buildings/house-5': { map: 'tutorial_006', center: '20,11', label: 'Home — level 5', fog: 'off' },
...
export function galleryMapDestination(
  ...
  const params =
    'scene' in location
      ? new URLSearchParams({ scene: location.scene, assets: 'custom' })
      : new URLSearchParams({
          map: location.map,
          assets: 'custom',
          intro: 'off',
  ...
  if (asset.kind === 'character' && appearanceId !== undefined) params.set('customHead', appearanceId);
  const note =
