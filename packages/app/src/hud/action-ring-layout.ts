 ...
 *
 * Original behaviour (research notes `Source/an original routine/CGuiManager.cs`): selecting 1–2 humans brings
 ...
 * + step, the ∓5 corner nudge, the centring around the unit) — the part recoverable from the analyse.
 * What the analyse does not recover: which command maps to which gfx id (the engine's
...
/** Step between adjacent buttons in a group (the analyse's `stepX/stepY = 0x20`). */
export const ACTION_STEP_PX = 0x20;
/** Arm offset from centre for the four cardinal groups (the analyse's `±100`). */
...
/** First/last-in-group corner nudge (the analyse's `±5` `cornerBias`). */
 ...
 * (clockwise from the top-left button), so this is the command→icon binding research notes' unfilled
