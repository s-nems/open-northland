...
/** The lip's opacity, unique within a gauge draw so a test can tell the lip from the fill strips. */
export const GAUGE_LIP_ALPHA = 0.9;

/** The panel's inner-box dark/light bevel lines, so the gauge outline matches the rest of the framing. */
export interface GaugeBevel {
  readonly dark: number;
  readonly light: number;
}

/** The gauge fill colour at `clamped` (0..100): the decoded ramp's entry, else the banded fallback. */
...
/** Blend `from` toward `to` by `t` (0..1), per RGB channel. */
