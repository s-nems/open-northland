...
/** The decoded original body one own package replaces, with its cast shadow when the atlas pairs one. */
export interface OriginalBuilding extends DecodedFrame {
  readonly shadow?: DecodedFrame | undefined;
}

export type OriginalBuildingOf = (manifest: OwnBuildingManifest) => OriginalBuilding | undefined;

/**
 * Load the original building bodies of the tribes the own packages skin, resolved per package through the
