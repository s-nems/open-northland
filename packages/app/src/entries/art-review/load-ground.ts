...
import { ownGrassBindings, ownMapPatch } from './map-patch.js';
...
export async function loadReviewGround(
  ...
  const ownGrass = mapGrassTexture(renderer, grass);
  const patterns = ownGrassBindings(ir.gfxPatterns ?? [], ownGrass.width, ownGrass.height);
  const x = Number(params.get('artX') ?? 130);
  const y = Number(params.get('artY') ?? 70);
  const patch = ownMapPatch(map, { x, y, width: 18, height: 20 }, patterns);
  return {
    terrain: patch.terrain,
    textures: {
      pages: new Map([['own-grass', ownGrass.source]]),
      cellFor: () => undefined,
