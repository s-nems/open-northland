import type { LivestockHeart } from '@open-northland/render';
import type { WorldSnapshot } from '@open-northland/sim';
import { PLAYER_SWATCH_COLORS } from '../../catalog/roster.js';
import { actorsOf, isSettler, ownerPlayerOf, positionOf, settlerTribeOf } from '../../game/snapshot.js';
 ...
 * `Owner`) becomes one faction-coloured heart the render {@link LivestockHeart} layer floats over its
 * back - the original's "the claimed sheep shows the owner's life heart" read. Pure over the snapshot
 * plus two seams: `isLivestockTribe` (the sim content's catchable-species classification, resolved by
 * the caller) and the roster's slot-to-colour mapping (the minimap-dot rule: `playerColourOf` picks
 * the swatch slot, {@link PLAYER_SWATCH_COLORS} names the RGB).
 */
export function computeLivestockHearts(
  ...
  for (const e of actorsOf(snapshot)) {
    ...
    out.push({ id: e.id, x: pos.x, y: pos.y, colour });
  }
  return out;
}
