 ...
 * project the door node of every building that has a tally. The door node is the building's anchor node
 * plus its type's `footprint.door` half-cell offset ({@link interactionNode}'s app-side twin), converted
 * back to a fixed-point `Position` the render layer projects with the same iso math as the building sprite.
 */
