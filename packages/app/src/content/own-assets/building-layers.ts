...
export async function loadOwnBuildingLayers(
  ...
  async function load(sprite: string, timeMask?: string): Promise<SpriteLayer> {
    ...
    const layer = { source: texture.source, atlas: ownBuildingAtlas(manifest) };
  ...
  const body = await load(manifest.sprite);
