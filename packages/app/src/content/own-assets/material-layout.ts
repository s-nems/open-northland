...
import { ownMountainPattern } from './mountain-layout.js';
...
export function ownMaterialBindings(
  patterns: readonly GfxPattern[],
  transitions: readonly GfxPatternTransition[],
  materials: readonly OwnTerrainMaterial[],
  ...
  for (const material of materials) {
    ...
    for (const row of patterns) {
      ...
      if (material.layout === 'mountain') {
        const binding = ownMountainPattern(row, pageKey);
      ...
      if (ground.has(row.editName)) throw new Error(`Duplicate own terrain binding: ${row.editName}`);
