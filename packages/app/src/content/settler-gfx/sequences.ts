...
/**
 * The other atomic ids the sim issues, transcribed from the sim's `settlers/atomics/start.ts` (eat 10 / sleep
 * 8 / pray 12 - themselves pinned to the original's `setatomic` table). Kept here rather than imported from
 * sim because they are the animation table's `byAtomic` keys - the same numeric contract the original's
