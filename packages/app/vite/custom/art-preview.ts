...
const alias = '@custom-art-preview';
...
export async function artPreviewPlugin(repoRoot: string, configuredPath: string): Promise<Plugin> {
  ...
  const match = /^\.art-build\/([a-z0-9][a-z0-9-]*(?:\/[a-z0-9][a-z0-9-]*)*)\/preview\/custom$/.exec(local);
  if (!match || normalizePath(await realpath(preview)) !== normalizePath(preview))
    throw new Error('ART_CANDIDATE must point to .art-build/<id>/preview/custom; run art preview first');
  ...
  const customRoot = normalizePath(resolve(root, 'packages/app/src/assets/custom'));
  // The public game reads the shared UI from its mirror, so a UI candidate must replace that too.
  const sharedUiRoot = normalizePath(resolve(root, 'packages/app/src/assets/ui'));
  ...
  return {
    ...
    async transform(code, id) {
      id = normalizePath(id);
      if (!id.startsWith(sourceRoot) || !/\.ts$/.test(id) || !/assets\/(custom|ui)\//.test(code)) return;
      ...
      function visit(node: unknown): void {
        ...
        if (
          ...
          const target = path.startsWith(`${customRoot}/`)
            ? `${alias}/${normalizePath(relative(customRoot, path))}`
            : path.startsWith(`${sharedUiRoot}/`)
              ? `${alias}/ui/${normalizePath(relative(sharedUiRoot, path))}`
              : undefined;
          if (target !== undefined)
