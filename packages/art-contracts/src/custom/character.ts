...
export const customCharacterShadow = z
  .object({
...
export type CustomStoredClip = z.infer<typeof storedClip>;
...
export function customCharacterFrameCount(m: {
...
export const customCharacterManifest = z
  .object({
    ...
    shadow: customCharacterShadow.optional(),
...
export type CustomCharacterManifest = z.infer<typeof customCharacterManifest>;

const appearanceId = z.string().regex(/^[a-z0-9-]+$/);
export const customCharacterSelection = z
  .array(appearanceId)
  .refine((ids) => new Set(ids).size === ids.length, 'Duplicate selected appearance');
export const customCharacterJobSelection = z.record(z.string().min(1), appearanceId);
