...
// PLACEMENT — the can-this-building-go-here check: the original's FREE placement rule (collision +
// a minimum distance encoded by the extracted footprint) evaluated over the world's obstacle/exclusion
// cells, in a sparse string form for the one-shot command gate and a dense mask form for the per-frame
// overlay probe, both stamped from ONE {@link eachBlockerCell} pass so they can never disagree.
