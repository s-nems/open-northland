 ...
 * ({@link musterAround}), then judge the launch ({@link decideWave}). Until the peace ends
 * ({@link peaceEndsAt}) the army only gathers at the door, as it does with no target. The men at the door
 * march when the muster is the wave it was gathering and the target's owner does not outnumber the seat; a
 * body already nearer the objective goes in whatever the muster says, having nowhere safe to wait;
 * everybody else is called in.
 */
