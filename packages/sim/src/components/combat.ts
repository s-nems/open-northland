 ...
 * integer arithmetic with an exact `hitpoints <= 0` death test. Every settler carries one (defaulting to
 ...
 * valid enemy is in reach/sight. The AISystem skips economy planning for an engaged unit (the
 * {@link PlayerOrder}-skip pattern) but sits below the needs drives, so hunger/fatigue/piety still pull it
 * away. `repathAt` throttles the chase: a chaser re-issues its walk toward a moving enemy only every
 * {@link REPATH_CADENCE} ticks, following its live path between, so a field of chasers never triggers a
 * per-tick full re-path (golden rule 7).
 *
 * Optional, on owned combatants only (wildlife fights in place). `repathAt` is a monotonic integer tick.
 */
 ...
 * content-relation behavior. `mode` is a small integer id.
 */
 ...
 * with no threat in sight carries no `Fleeing`. Stamped when a threat enters sight, removed after the
 * cool-down. The unit moves at its normal pace (there is no run/sprint gait); the drive only steers. The
 * CombatSystem uses `repathAt` to recompute the flee destination every few ticks rather than per tick
 * ({@link Engagement}'s throttle, golden rule 7), and the AISystem skips need-scheduling while it is present.
 *
 * `calmUntil` is the cool-down clock: null while a threat is in sight, set to `tick + cool-down` when the
 * last threat leaves, and reset to null if a threat reappears; reaching it ends the drive. Optional, on
 * actively-fleeing owned units. Both ticks are monotonic integers.
 */
export const Fleeing = defineComponent<{ repathAt: number; calmUntil: number | null }>('Fleeing');

/**
 * An explicit attack order the `attackUnit` command stamps on an owned combatant (the combat twin of
 * {@link PlayerOrder}). Unlike auto-engagement, which re-acquires the nearest enemy in sight each tick, an
 * ordered unit chases this specific `target` regardless of sight radius until it dies or becomes invalid —
 * then the CombatSystem drops the order and the unit reverts to auto-engagement. A move/profession order, or
 * the target becoming un-hostile, supersedes it.
 *
 * Optional; `target` is an {@link Entity} id.
 */
 ...
 * the `projectileSystem` advances each tick, so per-tick cost scales with active projectiles (golden rule 7).
 *
 * The payload is resolved once at launch so the flight is a pure advance with no content lookup mid-air:
 ...
 *    `projectileSystem` maps it onto a per-tick tile step via a named calibration constant, keeping the
 *    approximation in the system and the component faithful;
 *  - `originX`/`originY` — the shooter's Position at the release frame (fixed-point, frozen at launch). The
 *    flight never reads it; the render needs the chord's start to place the shot on its ballistic arc (the
 *    original visibly lobs arrows — observed, height approximated).
 *
 * Optional, on a bare entity with only a Position beside it. Every field is a whole integer or fixed-point.
 */
