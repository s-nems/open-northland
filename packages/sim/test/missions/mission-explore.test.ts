...
/** A scripted world under `mode` whose load pass has run once the fog was stamped, as a map's does;
 *  `setup` enqueues the fixture that stands before it. */
function underFog(
...
function revealing(results: readonly MissionResultOp[], mode: FogMode, setup?: (sim: Simulation) => void) {
  return underFog([firingMission(results)], mode, setup);
}

function asking(goal: MissionGoalOp, mode: FogMode, setup?: (sim: Simulation) => void): Simulation {
  return underFog([goalMission(goal)], mode, setup);
}
