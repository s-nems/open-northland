...
const VIKING_VEHICLES = [
  ...
  { typeId: 4, id: 'ship_big', jobId: 53, hitpoints: 5000, stockSlots: 200, passengerSlots: 9, logicSize: 2 },
  // a land cart - never a ship (passengerSlots 0), even though it is a vehicle.
  { typeId: 2, id: 'oxcart', jobId: 51, hitpoints: 1000, stockSlots: 30, passengerSlots: 0, logicSize: 0 },
  ...
  { typeId: 1, id: 'handcart', jobId: 50, hitpoints: 1000, stockSlots: 15, passengerSlots: 0, logicSize: 0 },
];
  ...
  it('is empty for a carts-only tribe - a cart is never a ship even when unlocked', () => {
    const sim = new Simulation({
      ...
      content: parseContentSet({
        ...
        buildings: [{ typeId: 1, id: 'headquarters', kind: 'storage' }],
        vehicles: [
          { typeId: 1, id: 'handcart', jobId: 50, hitpoints: 1000, stockSlots: 15, passengerSlots: 0 },
        ],
      }),
    });
    expect(tribeShipsUnlocked(sim.world, ctxOf(sim), TRIBE)).toEqual([]);
