...
describe('needsSystem - the computer seat refill', () => {
  it('writes a full bar over a computer settler’s critical hunger and fatigue on the seat’s minute turn', () => {
  ...
  it('leaves a bar above the critical mark alone, and never touches a human seat’s settler', () => {
    const sim = new Simulation({ seed: 1, content: testContent() });
    const computer = settlerWithHunger(sim, fx.fromInt(0));
    sim.world.add(computer, Owner, { player: COMPUTER_SEAT });
    const human = settlerWithHunger(sim, fx.fromInt(0));
