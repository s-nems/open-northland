...
describe('needsSystem - the setNeedsEnabled world rule (the dev/admin toggle)', () => {
  it('freezes every need while disabled and resumes on re-enable', () => {
    ...
    expect(sim.world.get(e, Settler).hunger).toBe(needBar(NEED_DRAIN_UNITS_PER_TICK));
  ...
  it('refuses the forge piety charge while disabled, so nothing sends a smith off to pray', () => {
    ...
    // The units a forge clip spends on the religion channel.
    ...
    chargeMilitaryPiety(sim.world, e, FORGE_PIETY_UNITS);
    expect(sim.world.get(e, Settler).piety).toBe(needBar(-FORGE_PIETY_UNITS));
  });
