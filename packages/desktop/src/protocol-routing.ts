 ...
 * `/bobs/<stem>.png` arrives as `app://bobs/<stem>.png` — the route segment lands in the URL host.
 * Folding such hosts back into the pathname makes both spellings hit the same route table. Kept
 * free of `electron` imports so the rule stays unit-testable.
 */
