...
`resources/icon.png`, `resources/icon.ico` and `resources/icon.icns` are generated by `npm run brand`
(`tools/brand`), never edited by hand: electron-builder's own downscaling drops transparency on
the small sizes and the prebuilt files carry the flat emblem there.
