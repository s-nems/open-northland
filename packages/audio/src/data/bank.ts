 ...
 * `Map` gets. It also folds in the terrain→ambient join (`typeId → bed names`) that the raw bank can't
 * express on its own: the bank keys ambient beds by pattern-GROUP name, while a map cell carries a
 * landscape `typeId`, so we bridge them through the IR's `terrainPatterns` (typeId → representative
 * `GfxPattern`) and that pattern's `editGroups`.
 */
