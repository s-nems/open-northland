 ...
 *  -5 dB (byte evidence: the master-volume conversion in `GameMp.exe`). */
 ...
 * fx-volume conversion in `GameMp.exe`). Deviation: 0 mutes fully, where the original floors at
 * -20 dB.
 */
 ...
 *  `GameMp.exe` fades the music audiopath by that much while a jingle rings). */
