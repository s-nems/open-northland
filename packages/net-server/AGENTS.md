...
- Never interprets a command payload. It reads an envelope's authority half, stamps the seat from the
  connection, and forwards the rest byte for byte; every client validates with the sim's parser.
