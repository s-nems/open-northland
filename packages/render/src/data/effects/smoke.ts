...
/** Concurrent puffs per emitter (phase-staggered thirds of the loop, so the plume never gaps). */
export const SMOKE_PUFFS_PER_EMITTER = 3;
 ...
 *  (kept off the edges), `v` down from the top into the roof band. Seeded per building, so each ruin
 *  smokes from its own spots — stable across frames (no per-frame jitter). */
export function emitterSpot(seed: number, i: number): { u: number; v: number } {
  return {
    u: 0.18 + 0.64 * frac(seed, i * 2),
...
export function smokePuff(seed: number, emitter: number, puff: number, tick: number): SmokePuffPose {
  ...
  return {
    ...
    alpha: Math.min(1, t * 5) * (1 - t) * SMOKE_PEAK_ALPHA,
  };
}
