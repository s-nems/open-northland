 ...
 * both sit above it under `data/`. Pure + total, and nothing here re-enters the sim.
 */
 ...
 * Materializes the filtered array for the once-per-snapshot HUD sum; the per-frame scene pick reads the
 * pairs in place via {@link isStockpileAmount} instead, so the hot path allocates nothing.
 */
