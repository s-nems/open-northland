...
reads the frozen snapshot + camera floats and never calls back into `sim` (root [`CLAUDE.md`](../../CLAUDE.md)
...
- **Bound the zoom-out** (`camera.ts` `MIN_ZOOM`). We deliberately do NOT support fitting a whole huge
...
Pixels + frame rate need a **human on a real GPU** (root `CLAUDE.md` point 4/5). Headless Chromium is
