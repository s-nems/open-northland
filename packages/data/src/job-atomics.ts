...
/** Memo keyed on the job table itself: a `ContentSet`'s `jobs` array is stable across callers. */
 ...
 * `allowedAtomics`, minus its own `forbiddenAtomics`. A base job absent from `jobs` contributes
 * nothing and a cycle stops at the repeated job; `validateCrossReferences` rejects both at load.
 */
