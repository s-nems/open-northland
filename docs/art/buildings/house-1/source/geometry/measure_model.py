...
rows=[]
for y in np.linspace(-.75,.75,13):
    sel=a[(np.abs(a[:,0])<.045)&(np.abs(a[:,1]-y)<.025)&(a[:,2]>.3)]
    rows.append({'y':float(y),'count':len(sel),'z95':float(np.percentile(sel[:,2],95)) if len(sel) else None})
report={'ridge_center_samples':rows,'vertex_bounds':[a.min(axis=0).tolist(),a.max(axis=0).tolist()], 'material_nodes':[{ 'name':m.name,'nodes':[(n.name,n.type) for n in m.node_tree.nodes]} for m in o.data.materials if m and m.node_tree]}
