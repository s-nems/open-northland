...
- [ ] **`.cif` decrypt + container parse.** Port `XB_Decrypt_Memory` from research notes
      `an original routine/XBTools.cs` (the `(in-1)^key` scheme, ~40 lines). *Decryption is solved;* the real
