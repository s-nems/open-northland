...
- Piety rise: `needsSystem` raises piety at a flat `PIETY_RISE_PER_TICK`=ONE/16384, but the original
  ...
  pray DRIVE (planner choosing `pray` at a piety threshold) is NOT yet landed — and unlike eat (store)
  and sleep (in place), it needs a need→satisfier→**temple-target** lookup (walk to a building) — that
  is the genuinely-new piece the next slice introduces. Strictly the right "devotion-lapses-then-pray"
  shape. The other target-bound needs (`enjoy` id 17 / `make_love` id 78) follow the same pattern.
