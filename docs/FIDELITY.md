...
| Production **per-cycle ticks** | approximated | `recipe.ticks` = `DEFAULT_RECIPE_TICKS`=20. The faithful per-cycle duration lives behind the produce atomic's animation `length`, reachable only through a tribe's `setatomic <job> <atomicId> "anim"` binding (per-tribe, last-wins — `atomicanimations` is keyed by name, not the numeric atomic id), and there is no tribe context at the building-type layer. Left at the schema default until a per-tribe recipe-timing pass resolves it. |
