...
| **Assets** (`.pcx`/`.bmd` → PNG/atlas) | **Yes — research notes pixel-oracle** | research notes renders the originals; diff decoded output pixel-for-pixel + decoder round-trip tests. (Pending an owned game copy + the oracle.) |
| **Sim behavior** (atomic planner, economy loops, AI, pathing, combat, atomic timings) | **No** | research notes' logic tick is a stub counter; the behavior is in neither the data nor the reverse-engineering. It can only be pinned by (a) the data-derived parameters, (b) the readable `.ini` *semantics*, (c) **calibration-by-observation** against the running original. Because nothing automatic catches drift here, **it must be tracked in this file.** |
...
| Decoded-asset **pixel** fidelity | not-verified | research notes pixel-diff not yet run (an agent can't self-judge; needs human + owned copy) |
...
| A\* pathfinding (canonical tie-break, per-tick budget) | approximated | Behavior has no oracle (research notes' logic tick is a stub). A\* + canonical tie-break is *our* deterministic choice; the engine's actual pather/path-cache is unknown. `PATHFINDING_BUDGET_PER_TICK`=8 is unpinned (calibration-by-observation pending). |
