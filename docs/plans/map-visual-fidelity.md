...
plays those at sim tick rate) and whether [GfxHouse] mirrors that shape. The research notes oracle
(../research notes) may document the GfxHouse fields — formats only, don't port code.
...
2. `fx wave*`: investigate what the engine does with them (research notes formats + the records'
