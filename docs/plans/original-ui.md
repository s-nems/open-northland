...
- [x] 5. App: settler order UI with original art — landed: `hud/action-ring-layout.ts` (radial geometry transcribed from research notes `BuildHumanActionButtons`) + `view/settler-actions.ts` (Pixi ring of `order_*` buttons, `context` palette) replacing the DOM actions card; buttons issue `setJob`/`setStance`. See FIDELITY.md "Settler action ring".
