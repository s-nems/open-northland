...
loose on disk); the HUD **layout** is hardcoded in `the original`, but research notes has the left panel
analysed to literal pixel rects + sprite ids, so geometry is transcribable, not guessed.
...
- GUI bobs are 8-bit indexed; the engine colorizes each element at draw time with a palette from
  ...
  pairs with which element: research notes `Source/an original routine/CGuiBaseDataManager.cs` (loads
  them all by name) and `Source/an original routine/CGuiManager.cs` (usage).
...
decoders against the research notes oracle; take format facts, never its architecture).
```
...
- Format: storable id 0x03F5 (`CFont`) — a thin wrapper around the same bob container the `.bmd`
  ...
  Oracle: research notes `Source/an original routine/CFont.cs` (+ `XBStorable.cs` for the id/version envelope):
...
- research notes gives ground truth for a subset: `Source/an original routine/CGuiManager.cs`,
...
3. Provenance notes in the map file header (or the SOURCES.md GUI section): which entries are
   pinned to research notes code vs human-identified vs unknown.

Process: harvest the research notes ids FIRST, then generate the montage, open it for the user,
...
app using the extracted GUI atlas, fonts, and the research notes-pinned geometry.

Context:
- Inputs from earlier steps (all in content/ via the pipeline): gui atlas + named frame map +
  .fnt fonts + ingamegui strings (pol/eng).
- Geometry ground truth: research notes `Source/an original routine/CGuiManager.cs` `Desktop_Open()`
...
- Original behavior (research notes `Source/an original routine/CGuiManager.cs`,
  ...
  `an original routine`, which is a TODO stub in research notes — the precise layout
...
- Original ground truth is thin here: the per-type windows exist in the engine
  ...
  are named-only in research notes, not ported — no geometry available. Compose the panel from the
