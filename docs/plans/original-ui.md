...
- [ ] 3. GUI sprite map (interactive — user is the oracle) — the map itself landed
  (`packages/app/src/content/gui-atlas-map.ts`: research notes-pinned ids + montage guesses);
...
deviation from the pinned original (research notes `CWorldDisplayElement` + `CBitmap.Tool_Darken`:
...
named approximation, research notes loads `PalBarHitpoints` but the draw site isn't ported; flat bands
...
packages/app/src/content/gui-atlas-map.ts (research notes-pinned entries + montage best guesses,
...
smallest pile — a single stone, a small wheat sheaf), NOT a code-pinned lookup (research notes has no
