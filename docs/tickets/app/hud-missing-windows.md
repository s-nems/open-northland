...
windows. Source basis: the `research notes`-provenance GUI-button table in `docs/SOURCES.md` (from
...
3. File one self-contained ticket per window the survey decides to pursue (context: decoded button
   ...
   research notes `CGuiManager.cs` `MiscWindows_*` for the window's contents as format evidence).
