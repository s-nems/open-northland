...
languages (`content/gui/strings/<lang>.json`, "eng + pol extracted" — docs/SOURCES.md, "Decoded GUI"
strings note) and
...
3. One locale switch seam: the existing `?lang=` param (`view/game-view.ts`) should select BOTH the
   decoded string tables and the clean-room catalog — today it never reaches the i18n layer (the
   *defaults* are already coupled: `DEFAULT_LOCALE` derives from `DEFAULT_UI_LANG` in
   `i18n/index.ts`; only the runtime selection is disconnected). Keep `pol` the default.
