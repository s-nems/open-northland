...
produce the validated sim `ContentSet`. Once the real-content chain wires `loadRealContent` into a
page that also renders (which needs `loadIr`), the multi-MB IR is JSON-parsed twice with two heap
copies — the browser HTTP cache dedups the network fetch, not the parse.
...
- Do this at the point real content is actually wired into a rendering entry
  ([real-content-switch](real-content-switch.md)), so the shared memo has two live callers — not before.
