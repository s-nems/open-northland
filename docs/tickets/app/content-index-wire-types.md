...
- `packages/app/src/entries/menu.ts` — `MapIndexEntry` (`id`, `name?`, `description?`, `minimap`),
  structurally identical to content-resolver' `MapsIndexEntry`.
- `packages/app/src/entries/icons.ts` — a local `BobsIndexEntry` (`stem`, `base`, `variant`),
  identical to content-resolver' export of the same name.
...
- Give `content-resolver` a browser-safe types-only entry point (e.g. a `./types` subpath export
