...
authoritatively `source:'research notes'`; 173 are `source:'montage'` best-guesses, many still named
`unknown_NNN`. The order-command icons (frames ~96–136, consumed by the action ring) can NEVER be
code-recovered: research notes' `sHumanCommandTypeToIconId` lookup is an unfilled placeholder — only
