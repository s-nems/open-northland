...
params — `runSlice(seed, mapCells?, map?, owner?, footprints?, goodNames?)` — where every caller must
...
- Replace the trailing optionals of `runSlice` / `runBareMap` / `runAuthoredSlice` with one options
  object (`{ mapCells?, map?, owner?, footprints?, goodNames? }`), keeping the required leading
  `seed`.
