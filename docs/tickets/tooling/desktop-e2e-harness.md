...
- A local-only e2e suite (like `test:content`: hard-fails without prerequisites, never runs in CI)
  ...
  - first-run wizard against `OPEN_NORTHLAND_DATA_DIR=<fresh dir>` - pick phase renders with the mod
    panel, a folder without `DataCnmd/` is refused, Cancel returns to pick;
  ...
  - a full real-mod conversion behind an explicit opt-in env var (it takes minutes).
...
The suite passes locally against the local mod; a deliberately broken protocol route or
