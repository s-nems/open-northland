...
performs, which the render then looks a choreography up by. Two of the source's 47 rows name an
...
Each has a program the worker should be playing: the smith's 60 and the joiner's 67. The tribe's own
`setatomic` rows name them (`viking_smith_produce_tool_iron` on 60,
`viking_carpenter_produce_spear_wooden` on 67), so the information is in the content; the good-keyed
lookup alone cannot reach it. (Water is not a craft case: the well's pump is the carrier's collect
action, `BuildingType.collectAtomic`.)
...
- Unit: a smith crafting `tool_iron` and a joiner crafting `spear_wooden` each resolve the atomic their
  tribe binds, and every ware whose row already agrees keeps its atomic.
- `npm run test:content`: no viking product resolves an atomic outside its maker's allowed set.
- Human pass: a staffed smithy and joinery each show their worker at the craft.
