...
lockstep game holds the same state, so any client can produce the save. What is missing is the
lifecycle around it in a networked session, and a terminal game state: the sim has none, the
...
- Save in game by any player: the file carries the session descriptor and the seat roster (nicks and
  seat modes, never tokens) so it can seed a new room.
...
- Late join and rejoin: a joiner into a running game, or a player returning after the countdown, gets
  the room's latest snapshot (server-cached when one exists, otherwise from a donor client) and the
  frames since, then catches up.
...
- Headless through the server: a client disconnects for 30 s, rejoins, and finishes with the majority
  ...
  joiner from a server-cached snapshot matches.
