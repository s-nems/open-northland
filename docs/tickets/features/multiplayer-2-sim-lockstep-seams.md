...
- Tick-targeted admission: an envelope may carry a target tick and an externally assigned sequence.
  `commandSystem` applies, at tick T, exactly the envelopes due at T, in ascending sequence, before
  the untargeted envelopes in enqueue order. An envelope whose target tick is already past is recorded
  as rejected, deterministically, never applied late. The replay log keeps recording `(applyTick,
  sequence)` so `stepReplaying`, `localizeDivergence`, and the diagnostics bundle work unchanged.
- A per-tick sync digest on `Simulation`, computed after `step()` from the tick, the RNG state, the
  entity counter and alive count, and the component values of the entities touched during that tick,
  folded per domain (a short fixed list of component groups, e.g. rng, entities, economy, settlers,
  movement, combat, fog) so a mismatch names the domain. It must read the touched set without
  consuming the log `takeSnapshot` drains, stay pure, and use the same canonical value encoder as
  `hashSimState` so the two cannot disagree about supported shapes. `hashState()` and every golden
  stay unchanged; the full hash remains the rare cross-check.
...
- Non-goals: no transport, no worker, no change to the save format, no change to `hashSimState`.
...
- Unit tests: a targeted envelope applies on exactly its tick and not before; two envelopes for one
  ...
  and the hash equals the run without it; untargeted envelopes keep next-tick semantics.
- Digest tests: perturbing the RNG, one fog mask, and one component each change only their domain and
  leave the other domains stable; two runs with the same inputs produce identical digest sequences; a
  run with diagnostics off does no digest work.
- Cost: on `npm run bench:map` the digest adds under 0.2 ms per tick at the 20k-tick window.
