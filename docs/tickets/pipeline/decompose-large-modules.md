...
Four `tools/asset-pipeline/src` modules are still over the ~300-line budget (`npm run
scan:structure`, `wc -l`) and fold several concerns into one file. Split each by domain concern into
...
`ByteCursor` and §5 relates `decodeBobFrame` to the pcx run codec — do those before or as part of
this split to avoid moving code twice.
