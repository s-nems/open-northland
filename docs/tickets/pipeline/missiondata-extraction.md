...
**Investigate first — the opcode structure is unknown.** A grep of `research notes/` for
`MissionData` finds nothing (it is file-format decoders with no game-logic layer), so the format
oracle likely does not cover trigger semantics — re-check for related logic (e.g. `an original routine`,
