...
`packages/audio/src/data/bindings.ts` binds only `projectileHit` (a launch sounds through the shot clip's
own authored cue), and the render leaves no mark. On screen a miss reads as the arrow popping out of existence.
...
Neither table is extracted: `ir.json` weapons carry no sound fields, so `byCombatWeapon` hand-picks an
impact by `weaponMainType` and a bear bite lands with `Weapon Sword Short Hit`. `weapons.ini` names the
species impacts itself (`BearFist` 102 "Bear Hit", `claw` 105 "Wolve Hit"), so extracting the two tables
answers the miss cue and the wrong-impact bite from one source.

## Scope

- Extract `soundtype_Hit` / `soundtype_NoHit` into the weapon rows, and resolve `combatHit` /
  `projectileHit` through them instead of the `weaponMainType` hand map.
