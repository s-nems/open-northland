...
2. Per-system timing: measure ms/tick per system across N ticks (warmup + measured window), report
   ...
   Timing instrumentation must live outside sim purity — inject a timer through the per-system
   instrumentation seam (`docs/tickets/sim/per-system-timing-seam.md` creates it; prefer landing
   that first and consuming it here) rather than putting `performance.now()` inside `packages/sim`
   (the hygiene test rejects nondeterministic globals there).
