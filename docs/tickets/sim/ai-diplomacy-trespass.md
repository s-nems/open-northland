...
Original (the original's `an original routine`, a hypothesis to confirm on the owned
`the original`): on the look at a neutral slot that does not hold the seat as enemy, the handler walks
that player's humans (`an original routine(player, 1)`) and asks
`an original routine` for each one's position. A human inside one of the seat's villages
...
`an original routine` has laid out the seat's villages.

This build's AI has no village areas, so the work starts with what a village covers
(`an original routine`) and which humans the iterator's flag 1 selects.
