...
Byte-level evidence from the owned copy's unencrypted `the original` (analysed in the
`research notes` analysis project, VAs below), cross-checked against the symbol-rich the original:
