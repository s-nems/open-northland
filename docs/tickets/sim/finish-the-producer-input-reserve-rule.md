...
**Area:** sim (agents/economy) · **Priority:** P2

`mayFetchGoodFrom` (`systems/agents/economy/store-policy.ts`) stops a consumer from lifting a good
...
`nearestStoreHolding` (`systems/agents/targets/stores/stock.ts`) accepts any positioned stockpile
...
- `agents/equip-order.ts` — an equip errand;
- `agents/assistant-grants.ts` — a granted item.
...
a slot that fills can only be drained by production. On the AI's own plan that parks roughly 15 wood
+ 15 iron per smithy, 10 + 10 in the joinery and 10 mud + 10 wood in the pottery. The visible
failure is "the joinery sits on 10 iron while the smithy shows 0 and idles", with no panel text
explaining it.
