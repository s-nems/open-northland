...
The digest stays sealed every tick: the disputed-tick forensics and the per-tick acknowledgement of
the [runtime architecture epic](../runtime-architecture/README.md) rest on it, so accumulating touched
sets across a window and sealing at its boundary is out of scope. The cost comes down through the split
above, not through fewer seals.
