...
- **Per-system share** — the report's `sharePct` is already scale-invariant; a gate could pin "no
  system exceeds X% of the tick". Cheap, but today's baseline (`ai` at ~89%) means the threshold
  would encode the very question that ticket is about — so this waits on it.
