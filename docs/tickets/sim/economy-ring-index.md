...
`ai-targets.ts` (`nearestStoreFor`, `nearestStoreHolding`, `nearestFoodStore`, `nearestTemple`,
`nearestConstructionSite`, `nearestWorkplaceOutput`, `nearestFreeYardNode`) and `ai-supply.ts`
(`nearestMissingInputSource`, `nearestGroundPile`, `nearestConstructionSiteNeeding`,
`boundProducerOutputToHaul`). `NodeBuckets.nearest` (`systems/spatial.ts`) already does the
canonical band search combat uses.
