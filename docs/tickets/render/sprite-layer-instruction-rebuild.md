...
isolation), the remaining dominant draw term is the sprite render group itself: any moving on-screen
unit re-writes zIndex, so Pixi re-sorts and re-builds that group's instruction set every frame
(live magiczny_las probe at tick ~24k, 161 drawn, 2048x1048: `_buildInstructions` ~0.98 ms +
`sortChildren` ~0.17 ms of the 2.38 ms running draw EMA; both drop to 0 when paused). The cost
scales with attached containers, so a big battle or city view multiplies it.
