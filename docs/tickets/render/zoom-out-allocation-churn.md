# Reduce zoomed-out allocation churn and GC hitches
...
Profile the zoomed-out frame with a real allocation profile (Chrome DevTools allocation
...
- `minimap drawDots` (`packages/app/src/hud/minimap/index.ts`) — rebuilds Graphics dots per frame;
...
Repeat the A/B measurement procedure above before/after; the
