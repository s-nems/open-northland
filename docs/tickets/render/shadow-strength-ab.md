...
(`(dst >> 1) & 0x7f7f7f`, research notes `CBitmap.cs`), and its managed `PrintBob_Shadow_TimeMask` uses
`shade = 128` → `dst × 0.5` — which would be alpha `0x80`, noticeably darker. research notes' type-2
