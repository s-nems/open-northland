...
- **Asset-decode correctness** — use the **research notes oracle**: research notes boots and renders the
...
- **Behavioral fidelity to the original** — the pyramid proves the sim is self-consistent and
  ...
  *Cultures*. There is no automatic mechanics oracle (research notes' logic tick is a stub). Faithfulness
  is pinned to the extracted data params, readable `.ini` semantics, research notes format evidence, or
