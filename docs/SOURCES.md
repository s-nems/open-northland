...
- **`.cif` is on the critical path — do NOT plan to avoid it.** Header begins `fd 03 00 00 ...`
  ...
  - The genuine unknown is therefore **not decryption but the decrypted payload/record layout**.
    research notes only ever decrypts `.cif` for language strings, so it does *not* hand you the parsed
...
pipeline against their own copy. GPLv3-compatible intent, mirroring research notes' stance.
