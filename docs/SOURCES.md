...
- `research notes/` — a C#/.NET binary-faithful reverse engineering. **Our format manual.**

## Original file formats (what the pipeline must decode)

Counts observed in `Cultures 8th Wonder` (base `Data` + `DataX` + mod `DataCnmd`):

| Ext | ~Count | What it is | Decode reference in research notes (`Source/`) |
|---|---|---|---|
...
| `.pcx` | 426 | palette-indexed pictures | `an original routine/CPicture.cs`, `an original routine/XBPictureTool.cs` |
| `.bmd` | 247 | "bob" framed sprite animations | `an original routine/CBobManager.cs` (3.8k lines), `an original routine/CBitmap.cs` |
| `.hlt` | 242 | lighting / remap tables | `an original routine/CRemapTable.cs`, `CHighColorCreator.cs`, `CTrueColorCreator.cs` |
| `.cif` | 167 | compiled/**encrypted** "Cultures Information File" (rules, maps) | decrypt: `an original routine/XBTools.cs` `XB_Decrypt_Memory`; also `an original routine/CIoHelper.cs`, `Dexter/DexMD5.cs` |
...
| `.fnt` | 63 | bitmap fonts (CFont `0x3F5` wrapping a CBobManager `0x3F4`) — the **in-game UI set** (12 fonts under `Data/gui/fonts/{,latin,rus}`) is **decoded** by `decoders/fnt.ts` + `stages/fonts.ts` (see "Fonts" below); the per-map briefing/hypertext `.fnt` are out of scope | `an original routine/CFont.cs` |
| `.lib` | — | packed archive (group + files + checksum) | `an original routine/CSimpleFileLibrary.cs` |
| file IO base | — | low-level readers, endian | `an original routine/CFile.cs`, `Dexter/DexterFile.cs`, `Dexter/DexterEndian.cs` |
| palettes | — | palette objects | `an original routine/CPalette.cs` |

### Key facts learned from inspection

- **`.cif` is on the critical path — do NOT plan to avoid it.** Header begins `fd 03 00 00 ...`
  (high-entropy). **Decryption is already solved** in research notes: `an original routine/XBTools.cs`
...
chunks** — the engine's `CIoHelper` container format (oracle: `an original routine/CIoHelper.cs`
...
- **Rules are largely declarative, behavior is not.** The type tables (`Data/logic/` +
  ...
  data nor reversed in research notes (its logic tick is a stub counter).
...
- **research notes status (as of its Feb 2026 commits):** boots an SDL window, decodes graphics,
...
- **`frame` / `bg_normal`** — window frames/borders + the normal window background (both **manager-loaded
  ...
  window/selection classes research notes hasn't ported, so the pairing is by asset+field name, not a quoted
...
- **`research notes`** (authoritative, from `CGuiManager.cs` `Desktop_Open`/`MiscButtons_*`/`MiscWindows_*`):
  ...
  statistics, `0x38` tech-tree (the analysis's `_btnHelp`/`_btnButtonN` field names are unreliable; the
...
- The **per-command order-icon gfx ids** are NOT recovered from code — research notes'
...
`decoders/fnt.ts` reuses `decoders/bmd.ts` wholesale. On disk (oracle: `an original routine/CFont.cs` +
`XBStorable.cs`):
...
**research notes as an oracle.** Beyond reading its code, *run* it: research notes boots and renders the
...
- **License: GPL-3.0-or-later** (`LICENSE` at the repo root; declared in `package.json`). Same
  family as OpenMW; mirrors research notes' stance.
...
- **Clean-room, not a port.** `research notes/` is consulted as *format documentation*
