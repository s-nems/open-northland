...
- [76dfbf1] A long-open "binary format not located" risk can be a *wrong-file* assumption: the map
  ...
  research notes oracle's IO/container helpers (`CIoHelper.cs` `SIoHelperChunk`) even though it "doesn't
...
- [fa70452] A `sim` mechanic with data-pinned *parameters* (atomic durations, job gates, stock caps)
  is NOT thereby `faithful` — its *behavior* (planner/loop shape) has no oracle (research notes' tick is
...
- [4b01c26] A decoded layer can carry MORE structure than the consumer wants: `map.dat` `lmlt` is the
  ...
  and classify it `approximated` in FIDELITY — the corner→cell rule has no behavioral oracle (research notes
...
- [c7f2657] An extracted magnitude param can be directionally AMBIGUOUS with no oracle — `animaltypes`
  ...
  and research notes' sim is a stub so it carries no semantics. The disambiguator was the data's OWN
