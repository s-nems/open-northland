...
import { ownArtPolicy, ownArtRoot } from '../own-art-policy.mjs';
...
const tracked = new Set([recipe, `${ownArtRoot}${image}`, source.provenance]);
...
function policy(c = catalog, d = delivery, shared = [source], read = readJson) {
  return ownArtPolicy(c, d, shared, read, (path) => tracked.has(path));
}
...
test('requires source provenance and matching recipe identity', () => {
  ...
  assert.throws(() => policy(catalog, delivery, [], () => ({ id: 'other', sourceBasis: 'own' })), /id/);
