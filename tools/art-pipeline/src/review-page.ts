...
if(m.cellWidth){const count=8*(m.walkFrames+m.idleFrames+[...m.atomicClips??[],...m.carryClips??[]].reduce((n,c)=>n+(c.poses===undefined?c.frames:0),0));const f=Math.min(i,count-1);return {x:f%m.columns*m.cellWidth,y:Math.floor(f/m.columns)*m.cellHeight,width:m.cellWidth,height:m.cellHeight,scale:m.scale};}
