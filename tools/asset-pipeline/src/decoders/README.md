...
| `mapdat.ts` ⏳ | `map.dat` `hoix`-chunk container | `an original routine/CIoHelper.cs` (`SIoHelperChunk` / `IO_File_Chunk_*`) — container + `lsiz` dims done; `pck`/`X8el` layer unpack pending |
