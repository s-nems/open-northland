 ...
 * be implemented by porting the FORMAT logic from the referenced research notes C# file.
 */
...
async function run(args: Args): Promise<void> {
  ...
  // 1. Unpack .lib archives                  -> ref research notes an original routine/CSimpleFileLibrary.cs
  // 2. Decode palettes + .hlt remap tables   -> ref an original routine/CPalette.cs, CRemapTable.cs
  // 3. Decode .pcx pictures -> PNG           -> ref an original routine/CPicture.cs, XBPictureTool.cs
  // 4. Decode .bmd bobs -> atlas + anim JSON -> ref an original routine/CBobManager.cs, CBitmap.cs  (hardest)
