 ...
 *    element is documented in `docs/SOURCES.md` (from the research notes `CGuiBaseDataManager`/`CGuiManager`
 ...
 * icons use `context`) per the research notes oracle, so `iconsleft` is the best single preview palette; the
 ...
 * research notes only shows the palette being loaded, not the draw-site pairing itself).
