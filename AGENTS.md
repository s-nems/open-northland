...
- `research notes/` — C#/.NET reverse engineering of original file formats. Use it as an
...
assets into the repo, do not port research notes source, and do not brand this project with the
...
4. **Prefer readable original sources.** The mod's `.ini` files under `DataCnmd/` are preferred when
   present; base-game plaintext `.ini` files are still better than encrypted `.cif`; research notes is
   the format oracle.
5. **Faithful first, with named approximations.** Tests prove self-consistency, not that a mechanic
   ...
   basis: extracted data, readable source semantics, research notes format evidence, or observed original
...
- The original game has no automatic sim oracle in research notes. When behavior is not data-pinned,
