...
- `research notes/` — C#/.NET binary-faithful reverse engineering of the original engine.
...
data (see `docs/TESTING.md`). (b) `research notes/` is **format documentation**, not code to
...
6. **Faithful first; deviations are deferred and recorded, never default.** The goal is a faithful
   ...
   whether it is *faithful*** — a different axis no test covers (research notes' sim is a stub, so
...
4. Rendering/visual change → an agent CANNOT self-judge pixels. Run the screenshot diff if present,
   otherwise say it needs a human. Validate decoded assets against the **research notes oracle**.
...
- **`tools/asset-pipeline/AGENTS.md`** — pipeline-only notes: prefer the mod's `.ini`, validate
  visual decoders against the research notes oracle, never commit decoded/copyrighted bytes.
