...
description: Reviews a Vinland diff for source-basis and faithfulness: mechanics and extraction should be pinned to original data, source semantics, research notes format evidence, or observed original behavior. Pass it the commit range or diff to review.
...
`../Cultures 8th Wonder/` for original/mod data and `../research notes/` for format evidence.
...
2. **Unstated source basis** — a mechanic, timing, binding, or extraction lands without saying whether
   it is pinned to extracted data, readable source semantics, research notes, or observation.
