...
- **Render / visual step → the visual-confirmation gate (a hard gate, not a soft flag).** A change
  ...
  "done" on green tests alone.** Automated tests and the research notes oracle prove it is *structurally*
  ...
  **Only the user can make that final call.** So:
  1. Do the work; get the automated gate (test/check/build) green; validate decoded assets against
     the research notes oracle; run the screenshot diff if one exists.
...
semantics, or the research notes format-oracle — or, if you consciously diverge or can't yet pin it,
**"approximated: \<what + why\>"**. There is no automatic mechanics oracle (research notes' sim is a
