...
const ITERATE_PLAYBOOK = '~/Projects/vikings/vinland/.claude/commands/iterate.md'
...
const REPORT_PATH =
  (argObj && argObj.reportPath) || '~/Projects/vikings/vinland/.claude/reports/iterate-supervisor.md'
...
Execute EXACTLY ONE iteration by reading and following the playbook at:
  ${ITERATE_PLAYBOOK}
Follow every step in it (working-tree check -> reflection check -> pick smallest step -> implement -> test -> first commit -> review -> address -> second commit). Obey AGENTS.md golden rules: the \`sim\` package is deterministic and pure, sim state is fixed-point ints, content-is-data, prefer the mod's .ini sources. This project commits DIRECTLY to main — do not create a branch.
